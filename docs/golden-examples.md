...
## 6. 日志按天轮转 + 惰性清理 — `services/log_service.py` (`add`/`_auto_cleanup`)

**示范模式**：写路径按天切分（`logs-YYYY-MM-DD.jsonl`）摊平单文件增长；维护上不设后台线程，
在写路径上每 N 次惰性触发——文件级整删过期天文件 + 条目级裁剪当天文件。

**为什么好**：单文件项目里省掉后台任务的生命周期管理（多 worker 下后台线程是竞态重灾区）；
按天切分让 `list(days=N)` 只读最近 N 天（慢查询根治），`每 200 条检查一次` 让维护成本与写入频率自适应。
