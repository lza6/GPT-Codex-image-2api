...
function normalizeConfig(config: SettingsConfig): SettingsConfig {
  ...
  return {
    ...
    alert_events: Array.isArray(config.alert_events) ? config.alert_events : ["circuit_breaker_open", "circuit_breaker_closed", "backup_failure", "backup_checksum_mismatch", "account_invalid", "account_recovered", "quota_exhausted", "quota_forecast_depletion"],
...
export const useSettingsStore = create<SettingsStore>((set, get) => ({
  ...
  saveConfig: async () => {
    ...
    try {
      ...
      // 第三方应用入口已移除，不再触发第三方应用更新事件
      toast.success("配置已保存");
      return true;
    } catch (error) {
      toastError(error, "保存系统配置失败");
      return false;
    } finally {
